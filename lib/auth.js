...
export async function signUp({ email, password, name, phone, userType }) {
  ...
  try {
    // Check if email already exists using RPC function (checks auth.users table)
    console.log('Checking if email already exists in auth.users...', { email })
    ...
    if (rpcError) {
      ...
      // Fallback to profiles table check if RPC fails
      ...
      if (existingProfile) {
        console.log('Email found in profiles table (fallback):', existingProfile)
        return { 
          data: null, 
    ...
    console.log('Email not found in auth.users, proceeding with signup...')
    ...
    if (authError) {
      ...
      const isDuplicateError = authError.message.toLowerCase().includes('user already registered') || 
                              authError.message.toLowerCase().includes('already been registered') ||
                              authError.message.toLowerCase().includes('already registered') ||
                              authError.message.toLowerCase().includes('duplicate') ||
                              authError.message.toLowerCase().includes('email address not available') ||
                              authError.message.toLowerCase().includes('user_already_exists') ||
                              authError.message.toLowerCase().includes('signup is disabled') ||
